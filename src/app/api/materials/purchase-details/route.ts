...
export async function DELETE(req: Request) {
  try {
    ...
    const res = await db.collection('material_purchases').deleteOne({ _id: new ObjectId(id) });
...
export async function PUT(req: Request) {
  try {
    ...
    const update = { ...body };
    delete update.id;
    const res = await db.collection('material_purchases').updateOne(
      { _id: new ObjectId(id) },
      { $set: update }
    );
  if (res.matchedCount === 0) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  const item = await db.collection('material_purchases').findOne({ _id: new ObjectId(id) });
  return NextResponse.json({ updated: item });
  } catch (err) {
    console.error('Purchase PUT failed', err);
    return NextResponse.json({ error: 'Failed' }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const { db } = await connectToDatabase();
    const inserted = await db.collection('material_purchases').insertOne({ ...body, date: body.date || new Date().toISOString() });
    const item = await db.collection('material_purchases').findOne({ _id: inserted.insertedId });
    return NextResponse.json(item);
  } catch (err) {
    console.error('Purchase POST failed', err);
    return NextResponse.json({ error: 'Failed' }, { status: 500 });
  }
}
